...
```{r}
used_cars_data$fuel_type[used_cars_data$fuel_type == ""] <- "electric"
```
...
```{r, echo = FALSE}
fuel_counts <- table(used_cars_data$fuel_type)
fuel_counts
```

```{r, echo = FALSE}
unique_fuel_types <- unique(used_cars_data$fuel_type)
quoted_values <- paste0("'", unique_fuel_types, "'")
quoted_values
```

```{r, echo=FALSE}
used_cars_data[used_cars_data$fuel_type == "",]
```
...
```{r, echo=FALSE}
used_cars_data[used_cars_data$engine == "–",]
```
...
```{r, echo=FALSE}
used_cars_data[used_cars_data$transmission == "–",]
```
...
```{r, echo=FALSE}
used_cars_data[used_cars_data$ext_col == "–",]
```
...
```{r, echo=FALSE}
used_cars_data[used_cars_data$int_col == "–",]
```
